...
a better approach? YES, lower REML than MODEL 1

```{r}
NWT_lme2 <- lmer(growth_rate ~ SPEI_3 * species_binomial + date.year + (1 | main), data = NWT_rate_c)

summary(NWT_lme2)
```

A Better better? YESS

```{r}
NWT_lme2_simplified <- lmer(growth_rate ~ SPEI_3 + species_binomial + date.year + (1 | main), data = NWT_rate_c)

summary(NWT_lme2_simplified)
```

GLM because my data is not normal distributed

```{r}
glmer_growth <- glmer(growth_rate ~ SPEI_3 + species_binomial + date.year + (1 | main), 
                      family = Gamma(link = "log"), data = NWT_rate_c)
