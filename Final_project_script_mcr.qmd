...
In DAG, it would look something like this: drought —\> growth , but in a effort to include more variables, I also recognize this relations: year —\> drought —\> \[ species —\> \] growth \<--- site
...
4.  Then, merged the two forest
    ...
    missing_columns <- setdiff(colnames(NWT_rate_c), colnames(BCI_rate_c))

    for (col in missing_columns) {
        BCI_rate_c[[col]] <- NA }


    BCI_rate_c <- BCI_rate_c[, colnames(NWT_rate_c)]

    extra_columns <- setdiff(colnames(BCI_rate_c), colnames(NWT_rate_c))
...
```{r}
NWT_rate_c_filtered$log_growth_rate <- log(NWT_rate_c_filtered$growth_rate + 1)  # Adding 1 to avoid log(0)


ggplot(NWT_rate_c_filtered, aes(x = SPEI_category, y = log_growth_rate, color = SPEI_category)) +
  geom_boxplot(alpha = 0.7) +
  labs(x = "SPEI Category (Drought Severity)", y = "Log Growth Rate") +
  theme_minimal() +
  ggtitle("Log Growth Rate Distribution by SPEI Categories")
...
```{r}
# Create categories for SPEI_3 (optional, based on your analysis)
NWT_rate_c_filtered$SPEI_category <- cut(NWT_rate_c_filtered$SPEI_3, 
                                          breaks = c(-Inf, -1, -0.5, 0), 
                                          labels = c("Severe Drought", "Moderate Drought", "Mild Drought"))

# Create the boxplot
ggplot(NWT_rate_c_filtered, aes(x = SPEI_category, y = growth_rate, color = SPEI_category)) +
  geom_boxplot(alpha = 0.7) +  # Boxplot for growth rate based on SPEI categories
  labs(x = "SPEI Category (Drought Severity)", y = "Growth Rate") +  # Axis labels
  theme_minimal() +
  ggtitle("Growth Rate Distribution by SPEI Categories (Drought Severity)") +
